...
const stttLogic = require('./games/superTicTacToe'); const pongLogic = require('./games/pong'); const c4Logic = require('./games/connect4'); const dabLogic = require('./games/dotsAndBoxes'); const bsLogic = require('./games/battleship'); const chkLogic = require('./games/checkers');
...
app.post('/register', async (req, res) => { 
    try { 
        ...
        const newUser = new User({ username, password: hashedPassword, rank: 100, profilePic: defaultPfp }); 
        await newUser.save(); 
...
// UPGRADED: Async initialization to fetch profiles and handle anonymity
async function initializeGame(roomName, chosenGame, player1, player2, privateCode = null) {
    ...
    const u1 = await User.findOne({ username: player1.username });
    const u2 = await User.findOne({ username: player2.username });

    // Apply Anonymity logic if it's a public game
    const getPlayerData = (user, fallbackName) => {
        if (!privateCode && user?.isAnonymous) {
            return { username: 'Anonymous', rank: '???', pfp: 'https://api.dicebear.com/7.x/identicon/svg?seed=Anon' };
        }
        return { username: user?.username || fallbackName, rank: user?.rank || 100, pfp: user?.profilePic || `https://api.dicebear.com/7.x/bottts/svg?seed=${fallbackName}` };
    };

    const p1Data = getPlayerData(u1, player1.username);
    const p2Data = getPlayerData(u2, player2.username);

    setTimeout(() => {
        const game = activeGames[roomName]; if (!game) return; 
        
        // Pass the rich player data instead of just names
        let safeGameState = { 
            roomName, gameType: chosenGame, isPrivate: !!privateCode, privateCode,
            playerData: { [player1.id]: p1Data, [player2.id]: p2Data } 
        };
        ...
        } else if (chosenGame === 'Pong') { game.players = { [player1.id]: 'Left', [player2.id]: 'Right' }; safeGameState.players = game.players; pongLogic.initPong(io, roomName, activeGames, handleGameEnd); }
